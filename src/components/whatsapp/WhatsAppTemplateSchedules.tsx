...
type DataSource = "resumo_eventos" | "resumo_cobranca" | "resumo_mgf" | "resumo_geral";
...
const SOURCE_LABEL: Record<DataSource, string> = {
  ...
  resumo_geral: "Resumo Geral (Consolidado — Eventos + Cobrança + MGF)",
};
...
const SOURCE_PATTERN: Record<DataSource, RegExp> = {
  ...
  resumo_geral: /geral|consolidad/i,
};
...
export function WhatsAppTemplateSchedules({ corretoraId }: Props) {
  ...
  const toggleAtivo = async (s: Schedule) => {
    const { error } = await supabase
      .from("whatsapp_template_schedules")
      .update({ ativo: !s.ativo, next_run_at: null })
      .eq("id", s.id!);
    if (error) toast.error("Erro: " + error.message);
    else load();
  };
  ...
  const runNow = async (id: string) => {
    ...
    if (error) toast.error("Erro: " + error.message);
      ...
      toast.error(data?.error || data?.results?.join(", ") || "Não foi possível entregar a mensagem");
      load();
    }
  };
      ...
      <CardHeader>
        <div className="flex items-center justify-between gap-2">
          <div>
            ...
            <CardDescription>
              Agende envios diários, semanais ou mensais usando templates Meta aprovados — sem precisar da janela de
              24h. As variáveis são preenchidas automaticamente com os dados do BI da associação.
            </CardDescription>
          </div>
          <Button onClick={openNew} disabled={!corretoraId}>
            ...
            {items.map((s) => (
              ...
              >
                ...
                <div className="flex gap-1">
                  <Button size="sm" variant="ghost" onClick={() => runNow(s.id!)} title="Enviar agora">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => remove(s.id!)} title="Excluir">
            ...
            <div className="space-y-3">
              <div>
                ...
                <Input
                  ...
                  placeholder="Ex: Resumo diário gestor"
                />
              </div>
              ...
              <div>
                ...
                >
                  <SelectTrigger>
                    <SelectValue
                      placeholder={
                        templates.length === 0 ? "Nenhum template aprovado disponível" : "Selecionar template..."
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(v) => setEditing({ ...editing, frequency: v as Frequency })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              {editing.frequency === "weekly" && (
                <div>
                  ...
                  <Select
                    value={String(editing.day_of_week ?? 1)}
              ...
              {editing.frequency === "monthly" && (
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setEditing({ ...editing, day_of_month: Math.max(1, Math.min(31, Number(e.target.value) || 1)) })
                    }
                  />
                  <p className="text-xs text-muted-foreground mt-1">
                  ...
                  {editing.recipients.map((r, i) => (
                    <div key={i} className="flex gap-2">
                      ...
                      {editing.recipients.length > 1 && (
                        <Button size="icon" variant="ghost" onClick={() => rmRecipient(i)}>
                          <Trash2 className="h-4 w-4 text-destructive" />
                        </Button>
                      )}
                    </div>
