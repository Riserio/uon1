...
async function getCobrancaTemplateBodyCount(name: string, language: string): Promise<number | null> {
  ...
  try {
    if (!wabaId) {
      const phoneRes = await fetch(`https://graph.facebook.com/v22.0/${metaPhoneId}?fields=whatsapp_business_account`, {
...
function parseMoneyValue(value: any): number {
  ...
  if (parsed >= 1000 && strParsed.endsWith(".00")) {
    ...
    if (
      parsed >= 10000 &&
      (strValue.includes(",00") || strValue.includes(".00") || /\d{4,}$/.test(strValue.replace(/[^0-9]/g, "")))
    ) {
  ...
  try {
    ...
    if (action === "start" && corretora_id) {
      ...
      if (githubRunIdStr) {
        ...
        if (existing?.id) {
          ...
          return new Response(JSON.stringify({ success: true, execucao_id: existing.id }), {
            headers: { ...corsHeaders, "Content-Type": "application/json" },
          });
        }
      }
      ...
      return new Response(JSON.stringify({ success: true, execucao_id: execucao.id }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (action === "error" && corretora_id) {
      ...
      if (GENERICO_RE.test(error_message || "") && execucaoIdCandidate) {
        ...
        if (exReal && exReal.status === "erro" && exReal.erro && !GENERICO_RE.test(exReal.erro)) {
          console.log("Cobranca: erro real ja registrado — ignorando fallback generico do workflow");
      ...
      if (config) {
        ...
        if (targetId) {
          ...
          const proximaTentativa =
            newRetryCount < MAX_RETRIES ? new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString() : null;
          ...
          if (proximaTentativa) {
            console.log(
              `[Webhook] Retry agendado para ${proximaTentativa} (tentativa ${newRetryCount}/${MAX_RETRIES})`,
            );
          } else {
            console.log(
              `[Webhook] Limite de retries atingido (${newRetryCount}/${MAX_RETRIES}) - não será feita nova tentativa automática`,
            );
          }
        } else {
          ...
          console.log(
            `[Webhook] Retry agendado para ${proximaTentativa} (nova execução com erro, tentativa 1/${MAX_RETRIES})`,
          );
        }
      ...
      return new Response(JSON.stringify({ success: true, message: "Erro registrado" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (update_progress) {
      ...
      if (!targetId) {
        return new Response(
          JSON.stringify({
            success: false,
            message: "Não foi possível identificar a execução para atualizar o progresso",
          }),
      ...
      return new Response(JSON.stringify({ success: true, message: "Progresso atualizado" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!corretoraId && corretora_slug) {
      ...
      if (corrError || !corretora) {
        return new Response(JSON.stringify({ success: false, message: "Corretora não encontrada" }), {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (!corretoraId) {
      return new Response(JSON.stringify({ success: false, message: "corretora_id ou corretora_slug é obrigatório" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!dados || !Array.isArray(dados) || dados.length === 0) {
      return new Response(JSON.stringify({ success: false, message: "Dados não fornecidos ou vazios" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (importacao_id) {
      ...
      if (exErr || !existing) {
        return new Response(JSON.stringify({ success: false, message: "importacao_id inválido" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (existing.corretora_id !== corretoraId) {
        return new Response(JSON.stringify({ success: false, message: "importacao_id não pertence à corretora" }), {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      if (
        typeof total_registros === "number" &&
        total_registros > 0 &&
        importacao.total_registros !== total_registros
      ) {
        await supabase.from("cobranca_importacoes").update({ total_registros }).eq("id", importacao.id);
      }
    ...
    console.log(
      `[Webhook Cobranca] ${allBoletos.length} brutos → ${boletosFiel.length} após dedup SGA (${removidosDedup} removidos)`,
    );

    // 3) Inserir em batches
    for (let i = 0; i < boletosFiel.length; i += BATCH_SIZE) {
      const boletosBatch = boletosFiel.slice(i, i + BATCH_SIZE);
      const { error: batchError } = await supabase.from("cobranca_boletos").insert(boletosBatch);
      ...
      if (targetExecucaoId && dados.length > 0) {
        ...
        const totalGeral = typeof total_registros === "number" && total_registros > 0 ? total_registros : dados.length;
    ...
    if (configId) {
      ...
      if (targetExecucaoId) {
        ...
        const totalGeral =
          typeof total_registros === "number" && total_registros > 0 ? total_registros : totalProcessados;
    ...
    if (isLastChunkFinal) {
      ...
      if (modo === "atualizar_anterior") {
        ...
        if (diaDoMes <= 6 && hojeDiaPermitido) {
          ...
          console.log(
            `[Webhook Cobrança] Dia ${diaDoMes} <= 6 mas dia da semana ${currentDayOfWeek} não é permitido para esta associação, pulando WhatsApp`,
          );
        }
      } else {
        ...
        if (diaDoMes >= 7 && hojeDiaPermitido) {
          ...
          console.log(
            `[Webhook Cobrança] Dia ${diaDoMes} < 7, mês atual será enviado a partir do dia 7 (mês anterior está sendo enviado)`,
          );
        } else if (!hojeDiaPermitido) {
          console.log(
            `[Webhook Cobrança] Dia da semana ${currentDayOfWeek} não é permitido para esta associação, pulando WhatsApp`,
          );
        }
      }
      ...
      if (deveEnviarWhatsApp) {
        try {
          ...
          if (waConfig?.envio_automatico_cobranca) {
            ...
            const phoneNumbers = (waConfig.telefone_whatsapp || "")
              .split(",")
              .map((p: string) => p.trim())
              .filter(Boolean);
            const metaToken = Deno.env.get("META_WHATSAPP_TOKEN");
            ...
            if (phoneNumbers.length > 0 && metaToken && metaPhoneNumberId) {
              ...
              for (const phone of phoneNumbers) {
                ...
                try {
                  ...
                  console.log(
                    `[Webhook Cobrança] Template WhatsApp enviado para ${formattedPhone}: ${metaResponse.ok ? "sucesso" : errorMessage}`,
                  );
                } catch (sendErr) {
              ...
              await supabase
                .from("whatsapp_config")
                .update({ ultimo_envio_automatico: new Date().toISOString(), ultimo_erro_envio: null })
                .eq("id", waConfig.id);
                ...
                try {
                  await supabase.functions.invoke("whatsapp-flow-engine", {
                    body: {
                      ...
                      flow_id: fluxoId,
                      trigger: "auto_import",
                      mes_referencia: mesRefParaRelatorio,
                    },
                  });
              ...
              for (const phone of phoneNumbers) {
                ...
                try {
                  ...
                  console.log(
                    `[Webhook Cobrança] WhatsApp enviado para ${formattedPhone}: ${metaResponse.ok ? "sucesso" : "erro"}`,
                  );
                } catch (sendErr) {
