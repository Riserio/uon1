...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const statusConfigChannel = supabase.channel('status_config_changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'status_config'
    }, () => {
  ...
  const loadCorretoras = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('corretoras').select('nome').order('nome');
      if (error) throw error;
  ...
  const loadStatusPrazo = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('status_config').select('nome, prazo_horas').eq('ativo', true);
      if (error) throw error;
  ...
  const handleSaveAtendimento = async (atendimento: Atendimento) => {
    try {
      ...
      if (!corretoraId && atendimento.corretora) {
        const {
          data: corretoraData
        } = await supabase.from('corretoras').select('id').eq('nome', atendimento.corretora).maybeSingle();
        if (!corretoraData) {
          // Create new corretora
          const {
            data: newCorretora,
            error: corretoraError
          } = await supabase.from('corretoras').insert({
      ...
      if (atendimento.contato) {
        const {
          data: contatoData
        } = await supabase.from('contatos').select('id').eq('nome', atendimento.contato).maybeSingle();
        if (contatoData) {
      ...
      if (atendimento.responsavel) {
        ...
        } = await supabase.from('profiles').select('id').eq('nome', atendimento.responsavel).maybeSingle();
        if (responsavelData) {
      ...
      if (existing) {
        ...
        const fieldsToTrack = ['assunto', 'status', 'prioridade', 'observacoes', 'data_retorno', 'responsavel_id', 'corretora_id', 'contato_id', 'tags'];
        const {
          data: oldData
        } = await supabase.from('atendimentos').select('*').eq('id', atendimento.id).single();
        if (oldData) {
          fieldsToTrack.forEach(field => {
            const oldValue = oldData[field];
            const newValue = dataToSave[field as keyof typeof dataToSave];
        ...
        const {
          error
        } = await supabase.from('atendimentos').update(dataToSave).eq('id', atendimento.id);
        if (error) throw error;
        ...
        const {
          data: newAtendimento,
          error
        } = await supabase.from('atendimentos').insert(dataToSave).select().single();
        if (error) throw error;
  ...
  const handleExportCSV = () => {
    const headers = ['ID', 'Corretora', 'Contato', 'Assunto', 'Prioridade', 'Responsável', 'Status', 'Tags', 'Observações', 'Criado em'];
    const rows = atendimentos.map(a => [a.id, a.corretora, a.contato || '', a.assunto, a.prioridade, a.responsavel, a.status, a.tags?.join('; ') || '', a.observacoes || '', new Date(a.createdAt).toLocaleString('pt-BR')]);
