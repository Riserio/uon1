...
export default function PIDDashboard({ corretoraId }: PIDDashboardProps) {
  ...
  const chartData = useMemo(() => {
    ...
    // Variação da permanência mês a mês (% vs mês anterior)
    for (let i = 0; i < base.length; i++) {
      if (i === 0) {
        (base[i] as any).variacao_permanencia = 0;
      } else {
        const atual = (base[i] as any).permanencia || 0;
        const anterior = (base[i - 1] as any).permanencia || 0;
        if (anterior === 0) {
          (base[i] as any).variacao_permanencia = 0;
        } else {
          (base[i] as any).variacao_permanencia = ((atual - anterior) / Math.abs(anterior)) * 100;
        }
      }
    }
                    ...
                    <LineChart data={chartData}>
                      ...
                      <Tooltip
                        formatter={(value: any, name, props) => {
                          const key = (props as any).dataKey as string;
                          if (key === "variacao_permanencia") {
                            const v = Number(value) || 0;
                            const sinal = v > 0 ? "+" : v < 0 ? "" : "";
                            return [`${sinal}${v.toFixed(2).replace(".", ",")}%`, "Variação vs mês anterior"];
                          }
