...
export default function EstudoBaseHistoricoImportacoes({ onActivate, corretoraId }: Props) {
  ...
  const fetchImportacoes = async () => {
    if (!corretoraId) { setImportacoes([]); setLoading(false); return; }
    setLoading(true);
    try {
  ...
  useEffect(() => { fetchImportacoes(); }, [corretoraId]);
  ...
  const handleActivate = async (id: string) => {
    ...
    try {
      ...
      await supabase.from("estudo_base_importacoes").update({ ativo: false }).eq("corretora_id", corretoraId).neq("id", id);
      await supabase.from("estudo_base_importacoes").update({ ativo: true }).eq("id", id);
      await registrarLog({ modulo: "estudo_base", acao: "alteracao", descricao: `Importação ativada: ${importacao?.nome_arquivo}`, corretoraId });
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      await registrarLog({ modulo: "estudo_base", acao: "exclusao", descricao: `Importação excluída: ${importacao?.nome_arquivo}`, corretoraId });
                ...
                <TableRow key={imp.id}>
                  ...
                  <TableCell className="text-right">
                    <div className="flex items-center justify-end gap-2">
                      {!imp.ativo && (
                        <Button variant="outline" size="sm" onClick={() => handleActivate(imp.id)} disabled={activating === imp.id}>
                          {activating === imp.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <><RefreshCw className="h-4 w-4 mr-1" />Ativar</>}
                        </Button>
                      )}
                      <AlertDialog>
                        <AlertDialogTrigger asChild>
                          <Button variant="outline" size="sm" className="text-destructive hover:text-destructive" disabled={deleting === imp.id}>
                            {deleting === imp.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
                          </Button>
                        </AlertDialogTrigger>
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>
                              Excluir permanentemente "{imp.nome_arquivo}" e todos os {imp.total_registros?.toLocaleString('pt-BR')} registros.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancelar</AlertDialogCancel>
                            <AlertDialogAction onClick={() => handleDelete(imp.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                              Excluir
                            </AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
