    ...
    <Card className="bg-card/60 border-border/40 rounded-2xl">
      <CardContent className="p-0">
        ...
        >
          ...
          {!filtersOpen && activeCount > 0 && (
            <span className="text-[11px] text-muted-foreground truncate max-w-[45%]">
              {[
                ...
                filters.faixaValorProtegido !== "todos"
                  ? FAIXAS_VALOR.find((f) => f.value === filters.faixaValorProtegido)?.label
                  : null,
              ]
                .filter(Boolean)
                .join(" · ")}
          ...
          <ChevronDown
            className={`h-4 w-4 text-muted-foreground transition-transform duration-200 shrink-0 ${filtersOpen ? "rotate-180" : ""}`}
          />
        </button>
              ...
              {[
                {
                  ...
                  el: (
                    <Select
                      ...
                      onValueChange={(v) =>
                        onFiltersChange({ ...filters, situacao: v === "todas" ? [] : v.split(",") })
                      }
                    >
                ...
                },
                {
                  ...
                  el: (
                    <Select
                      ...
                      onValueChange={(v) => onFiltersChange({ ...filters, regional: v })}
                    >
                      <SelectTrigger className="h-8 text-xs rounded-lg">
                ...
                },
                {
                  ...
                  el: (
                    <Select
                      ...
                      onValueChange={(v) => onFiltersChange({ ...filters, montadora: v })}
                    >
                      <SelectTrigger className="h-8 text-xs rounded-lg">
                        <SelectValue placeholder="Todas" />
                      </SelectTrigger>
...
function buildFaixasValorDinamicas(maxValue: number) {
  ...
  for (let min = 0; min < maxFaixa; min += step) {
    const max = min + step;
    const label =
      min === 0 ? `Até ${(max / 1000).toFixed(0)}k` : `${(min / 1000).toFixed(0)}-${(max / 1000).toFixed(0)}k`;
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-6">
                  ...
                  <span className="text-[11px] tabular-nums min-w-[56px] text-right text-muted-foreground">
                    {formatPct(pct)}
                  </span>
                  <span
                    className={[
                      "text-[11px] tabular-nums min-w-[80px] text-right font-medium",
                      !hasVar ? "text-muted-foreground" : v! > 0 ? "text-emerald-600" : "text-destructive",
                    ].join(" ")}
                  >
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-6">
                  ...
                  <span className="text-[11px] tabular-nums min-w-[64px] text-right text-muted-foreground">
                    {formatPct(pct)}
                  </span>
                </div>
              </div>
...
export default function EstudoBaseDashboard({
  registros,
  loading,
  filters,
  onFiltersChange,
  ...
  return (
    <div className="space-y-3">
      ...
      {!hideFilters && (
        <Card className="bg-card/60 border-border/40 rounded-2xl">
          <CardContent className="p-0">
            ...
            >
              <Filter className="h-3.5 w-3.5 text-primary shrink-0" />
              <span className="font-semibold text-xs tracking-wide uppercase text-muted-foreground flex-1">
                Filtros
              </span>
              {!filtersOpen &&
                (() => {
                  ...
                  if (filters.faixaValorProtegido !== "todos")
                    parts.push(FAIXAS_VALOR.find((f) => f.value === filters.faixaValorProtegido)?.label || "");
        ...
        {[
          {
            ...
            value: totalPlacas.toLocaleString("pt-BR"),
            icon: Car,
          ...
          {
            label: "Com Eventos",
            value: totalComEventos.toLocaleString("pt-BR"),
            icon: ShieldCheck,
            cls: "text-violet-600 bg-violet-500/5 border-violet-500/20",
          },
          {
            label: "Total Base Sinistrada",
            value: `${taxaEventos.toLocaleString("pt-BR", { maximumFractionDigits: 2 })}%`,
            icon: TrendingUp,
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <Card className="lg:col-span-2 rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Placas por Situação</CardTitle>
          </CardHeader>
              ...
              <ResponsiveContainer width="100%" height={Math.max(120, placasPorSituacao.length * 38)}>
                <BarChart data={placasPorSituacao} layout="vertical" margin={{ top: 2, right: 64, bottom: 2, left: 4 }}>
                  ...
                  <Bar
                    ...
                    radius={[0, 6, 6, 0]}
                    maxBarSize={26}
                    background={{ fill: "hsl(var(--muted)/0.25)", radius: 6 }}
                  >
        ...
        <Card className="rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Tipo de Veículo</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        <Card className="lg:col-span-2 rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Valor FIPE por Faixa</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
                  ...
                  <BarChart data={valorProtegidoPorFaixa} margin={{ top: 18, right: 8, bottom: 4, left: 0 }}>
                    ...
                    <Bar dataKey="total" fill="hsl(var(--primary))" radius={[5, 5, 0, 0]} maxBarSize={36}>
                      <LabelList
                        ...
                        position="top"
                        formatter={(v: any) => Number(v).toLocaleString("pt-BR")}
                        style={{ fontSize: 9, fontWeight: 600, fill: "hsl(var(--muted-foreground))" }}
                      />
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
        ...
        <Card className="rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Combustível</CardTitle>
          </CardHeader>
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Card className="rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Top Montadoras</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
        ...
        <Card className="rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Top Modelos</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
            {modeloData.length > 0 ? (
              <div className="space-y-2 pt-1">
                ...
                <div style={{ height: 200 }}>
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={cadastrosPorMesChart} margin={{ top: 18, right: 12, bottom: 4, left: 0 }}>
                      ...
                      />
                      <Tooltip
                        formatter={(v: any) => [Number(v).toLocaleString("pt-BR"), "Cadastros"]}
                        contentStyle={ttStyle}
                      />
                ...
                <div className="overflow-y-auto max-h-[160px] pr-1 space-y-1">
                  {[...eventosPorMesChart].reverse().map((it) => (
                    ...
                    >
                      <span className="text-xs font-medium">{it.mes}</span>
                      <div className="flex items-center gap-6">
                        <span className="text-xs font-bold tabular-nums min-w-[52px] text-right text-violet-600">
                          {Number(it.comEvento).toLocaleString("pt-BR")}
                        </span>
                        <span className="text-xs tabular-nums min-w-[48px] text-right text-muted-foreground">
                          {Number(it.total).toLocaleString("pt-BR")}
                        </span>
                        <span className="text-[11px] font-semibold tabular-nums min-w-[56px] text-right text-rose-500">
                          {it.taxa.toFixed(2)}%
                        </span>
                      </div>
                    </div>
      ...
      <div className="grid gap-3 grid-cols-1 md:grid-cols-3">
        ...
        <Card className="rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Faixa Etária</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
            <div className="h-[200px]">
              {idadeData.length > 0 ? (
      ...
      <div className="grid gap-3 lg:grid-cols-2">
        <Card className="rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Voluntário</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
        ...
        <Card className="rounded-2xl border-border/40">
          <CardHeader className="pb-1 pt-4 px-5">
            <CardTitle className="text-sm font-semibold">Categoria</CardTitle>
          </CardHeader>
          <CardContent className="px-4 pb-4">
              ...
              {categoriaData.length > 0 ? (
                <BarChart data={categoriaData} layout="vertical" margin={{ top: 2, right: 56, bottom: 2, left: 4 }}>
                  ...
                  <Bar
                    ...
                    radius={[0, 5, 5, 0]}
                    maxBarSize={22}
                    background={{ fill: "hsl(var(--muted)/0.25)", radius: 5 }}
                  >
                    {categoriaData.map((e, i) => (
                      <Cell key={i} fill={e.fill} />
                    ))}
                    <LabelList
                      dataKey="value"
      ...
      <Card className="rounded-2xl border-border/40">
        <CardHeader className="pb-1 pt-4 px-5">
          <CardTitle className="text-sm font-semibold">Ano do Modelo</CardTitle>
        </CardHeader>
        <CardContent className="px-4 pb-4">
          <div className="h-[200px]">
      ...
      <Card className="rounded-2xl border-border/40">
        <CardHeader className="pb-1 pt-4 px-5">
          <CardTitle className="text-sm font-semibold">Regional / Cooperativa</CardTitle>
        </CardHeader>
        <CardContent className="px-4 pb-4">
