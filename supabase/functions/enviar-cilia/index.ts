...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Mapear UF para ID de região do CILIA
    const ufToRegionId: { [key: string]: number } = {
      "AC": 1, "AL": 2, "AP": 3, "AM": 4, "BA": 5, "CE": 6, "DF": 7, "ES": 8,
      "GO": 9, "MA": 10, "MT": 11, "MS": 12, "MG": 13, "PA": 14, "PB": 15, "PR": 16,
      "PE": 17, "PI": 18, "RJ": 19, "RN": 20, "RS": 21, "RO": 22, "RR": 23, "SC": 24,
      "SP": 25, "SE": 26, "TO": 27
    };

    // Mapear tipo de pintura
    const paintTypeMap: { [key: string]: string } = {
      "solida": "solid",
      "metalica": "metallic",
      "perolizada": "pearlescent"
    };
    ...
    const budgetPayload: CiliaBudgetRequest = {
      ...
      vehicleRegionId: ufToRegionId[vistoria?.veiculo_uf || ""] || 25, // Default SP
      licensePlate: vistoria?.veiculo_placa || "",
      insuredValue: vistoria?.veiculo_valor_fipe || 0,
      mileage: vistoria?.quilometragem || 0,
      paintType: paintTypeMap[vistoria?.tipo_pintura || "solida"] || "solid",
