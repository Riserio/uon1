...
import { Bell, Send, Settings, History, Users, MapPin, Building2, Loader2, CheckCircle2, XCircle, Globe, Image as ImageIcon, Clock, BookmarkPlus, LayoutTemplate, Trash2, Pencil, RefreshCw, Smartphone, Mail, MessageSquare, ChevronDown } from "lucide-react";
...
function formatarDataUnix(segundos: number | null | undefined): Date | null {
...
export default function PushCentral() {
  ...
  const [editStatus, setEditStatus] = useState("enviado");
  ...
  const loadAssinantes = async () => {
    ...
    try {
      const { data, error } = await supabase.rpc("listar_push_assinantes" as never);
      ...
      setAssinantes(parsed);
    } catch (e: unknown) {
      toast.error(mensagemDeErro(e, "Erro ao carregar assinantes"));
      setAssinantes([]);
    } finally {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (c) {
        ...
        setAtivo(!!c.ativo);
        // Se já está tudo configurado, começa recolhido pra não ocupar
        // espaço à toa; se falta algo, abre sozinho pra chamar atenção.
        setConfigOpen(!(c.ativo && c.onesignal_app_id && c.onesignal_rest_api_key));
      } else {
        setConfigOpen(true);
      }
  ...
  const abrirEdicaoEnvio = (envio: Envio) => {
    setEditingEnvio(envio);
    setEditTitulo(envio.titulo);
    setEditMensagem(envio.mensagem);
    setEditStatus(envio.status);
    setEditErro(envio.erro || "");
  };

  const salvarEdicaoEnvio = async () => {
    if (!editingEnvio) return;
    if (!editTitulo.trim() || !editMensagem.trim()) {
      toast.error("Título e mensagem não podem ficar vazios");
      return;
    }
    setSavingEdit(true);
    try {
      ...
      toast.error(mensagemDeErro(e, "Erro ao editar histórico"));
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  const excluirEnvio = async (id: string) => {
    setExcluindoId(id);
    try {
      const { error } = await supabase.from("push_envios" as never).delete().eq("id", id);
      if (error) throw error;
      toast.success("Registro removido do histórico");
      loadEnvios();
    } catch (e: unknown) {
      toast.error(mensagemDeErro(e, "Erro ao remover registro"));
    } finally {
      setExcluindoId(null);
    }
  };
  ...
  return (
    <div className="space-y-4">
      {/* Configuração OneSignal (colapsável — só precisa ficar aberta pra mexer na config) */}
            ...
            <button type="button" className="w-full text-left">
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <ChevronDown className={`h-4 w-4 text-muted-foreground shrink-0 transition-transform ${configOpen ? "rotate-180" : ""}`} />
                </div>
                <CardDescription>
      ...
      <Tabs
        defaultValue="enviar"
        onValueChange={(v) => {
          if (v === "assinantes" && assinantes === null) loadAssinantes();
        }}
      >
        <TabsList>
            ...
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between gap-2">
                <div>
                  ...
                  <CardDescription>
                    Dispositivos, e-mails e números inscritos no OneSignal para esse app.
                  </CardDescription>
                </div>
                <Button variant="outline" size="sm" onClick={loadAssinantes} disabled={loadingAssinantes} className="gap-1.5">
                  {loadingAssinantes ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <RefreshCw className="h-3.5 w-3.5" />}
                  Atualizar
                </Button>
              </div>
            </CardHeader>
            <CardContent>
              {loadingAssinantes && assinantes === null ? (
                    ...
                    <div key={e.id} className="flex items-start gap-3 rounded-xl border border-border/40 p-3">
                      ...
                      <div className="text-right shrink-0 space-y-1">
                        ...
                        <div className="flex items-center justify-end gap-1 pt-1">
                          <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => abrirEdicaoEnvio(e)} aria-label={`Editar envio ${e.titulo}`}>
                            <Pencil className="h-3 w-3" />
                          </Button>
                          <AlertDialog>
                            <AlertDialogTrigger asChild>
                              <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-red-500" aria-label={`Excluir envio ${e.titulo}`} disabled={excluindoId === e.id}>
                                {excluindoId === e.id ? <Loader2 className="h-3 w-3 animate-spin" /> : <Trash2 className="h-3 w-3" />}
                              </Button>
                            </AlertDialogTrigger>
                            <AlertDialogContent>
                              <AlertDialogHeader>
                                ...
                                <AlertDialogDescription>
                                  Isso remove só o registro "{e.titulo}" do histórico — não cancela nem reenvia nada no OneSignal.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter>
                                <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                <AlertDialogAction onClick={() => excluirEnvio(e.id)}>Excluir</AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
                          </AlertDialog>
                        </div>
      ...
      {/* Dialog de edição do histórico */}
      <Dialog open={!!editingEnvio} onOpenChange={(open) => !open && setEditingEnvio(null)}>
        <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <Input id="edit-titulo" value={editTitulo} onChange={(e) => setEditTitulo(e.target.value)} maxLength={80} />
            </div>
            <div className="space-y-1.5">
              <Label htmlFor="edit-mensagem">Mensagem</Label>
              <Textarea id="edit-mensagem" value={editMensagem} onChange={(e) => setEditMensagem(e.target.value)} maxLength={300} rows={3} />
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="edit-erro">Erro (opcional)</Label>
                <Input id="edit-erro" value={editErro} onChange={(e) => setEditErro(e.target.value)} placeholder="Sem erro" />
              </div>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingEnvio(null)}>Cancelar</Button>
            <Button onClick={salvarEdicaoEnvio} disabled={savingEdit} className="gap-2">
