...
export function HistoricoDialog({ atendimentoId, open, onOpenChange }: HistoricoDialogProps) {
  ...
  const formatValue = async (value: any, fieldName: string): Promise<string> => {
    ...
    // Verificar se é um UUID e buscar o nome correspondente
    if (isUUID(valueStr)) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[85vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            Visualize todas as alterações realizadas neste atendimento
          </DialogDescription>
        </DialogHeader>
