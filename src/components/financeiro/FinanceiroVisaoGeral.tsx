...
import { TrendingUp, TrendingDown, BarChart3, PieChart as PieChartIcon, Target, Wallet, FileText, ArrowUpRight, ArrowDownLeft } from "lucide-react";
...
export default function FinanceiroVisaoGeral({
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      let query = supabase.from("lancamentos_financeiros").select("*").order("data_lancamento", {
        ascending: false
      });
      ...
      if (lancamentos && lancamentos.length > 0) {
        ...
        const receitasMes = lancamentos.filter(l => {
          ...
          return l.tipo_lancamento === "receita" && date.getMonth() === currentMonth && date.getFullYear() === currentYear;
        }).reduce((sum, l) => sum + (l.valor_liquido || 0), 0);
        const despesasMes = lancamentos.filter(l => {
          const date = new Date(l.data_lancamento);
          return l.tipo_lancamento === "despesa" && date.getMonth() === currentMonth && date.getFullYear() === currentYear;
        }).reduce((sum, l) => sum + (l.valor_liquido || 0), 0);
        setKpis({
          receitaTotal,
          despesaTotal,
        ...
        setMonthlyData(Object.entries(monthlyMap).map(([name, values]) => ({
          name,
          ...values
        })));
        ...
        setCategoryData(Object.entries(categories).map(([name, values]) => ({
        ...
        setStatusData([{
          ...
          name: "Pago",
          value: statusCounts.pago,
          fill: "#16a34a"
        }, {
          name: "Rejeitado",
          value: statusCounts.rejeitado,
          fill: "#dc2626"
        }].filter(s => s.value > 0));

        // Balance evolution
        let runningBalance = 0;
        setBalanceData(Object.entries(monthlyMap).map(([name, values]) => {
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card className="bg-gradient-to-br from-blue-500/10 to-blue-500/5 border-blue-500/20">
          <CardContent className="p-5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-muted-foreground">Saldo Projetado </p>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={350}>
            <ComposedChart data={monthlyData}>
              ...
              <Line type="monotone" dataKey="saldo" stroke="#3b82f6" strokeWidth={2} name="Saldo" dot={{
              fill: '#3b82f6'
            }} />
            </ComposedChart>
          </ResponsiveContainer>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <AreaChart data={balanceData}>
                ...
                <Area type="monotone" dataKey="saldo" stroke="hsl(var(--primary))" fillOpacity={1} fill="url(#colorSaldo)" name="Saldo Acumulado" strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={statusData} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={2} dataKey="value" label={({
                name,
                percent
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            <div className="space-y-3">
              {recentLancamentos.map(l => <div key={l.id} className="flex items-center justify-between p-3 rounded-lg bg-muted/50 hover:bg-muted transition-colors">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${l.tipo_lancamento === 'receita' ? 'bg-green-500/10' : 'bg-red-500/10'}`}>
                      {l.tipo_lancamento === 'receita' ? <TrendingUp className="h-4 w-4 text-green-600" /> : <TrendingDown className="h-4 w-4 text-red-600" />}
                    </div>
                    <div>
                  ...
                  <div className="text-right">
                    ...
                    <Badge variant={l.status === 'aprovado' || l.status === 'pago' ? 'default' : l.status === 'pendente' ? 'secondary' : 'destructive'} className="text-xs">
                      {l.status}
                    </Badge>
                  </div>
                </div>)}
              {recentLancamentos.length === 0 && <p className="text-center text-muted-foreground py-8">
