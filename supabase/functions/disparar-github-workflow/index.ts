...
async function startCobrancaApiImport(supabase: any, corretoraId: string, user: RequestUser): Promise<{ started: boolean; message: string }> {
  ...
  // Agenda a importação via pg_cron para evitar timeout do gateway (a RPC pode rodar por vários minutos).
  // O job roda dentro do Postgres, comita normalmente e atualiza execução/config ao final.
  ...
  if (schedErr) {
    ...
    await supabase.from("cobranca_automacao_execucoes").update({
  ...
  try {
    ...
    if (action === 'dispatch' || !action) {
      ...
      if (apiCredC?.usar_api && apiCredC?.api_token) {
        // Fallback GitHub Actions desativado globalmente para Cobrança: sempre via API (assíncrono).
      ...
      // Fallback GitHub Actions desativado para Cobrança: exige API habilitada.
      return new Response(
        JSON.stringify({ success: false, message: "Importação de Cobrança agora é exclusiva via API Hinova. Habilite 'Usar API' nas credenciais da associação." }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
      ...
      if (!githubPat || !githubRepoOwner || !githubRepoName) {
        ...
        return new Response(
          JSON.stringify({
            success: false,
            message: "Configuração do GitHub incompleta. A API não está disponível para esta associação e o fallback exige GitHub configurado.",
          }),
          { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } },
        );
      }
