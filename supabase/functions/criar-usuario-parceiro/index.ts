  ...
  try {
    ...
    if (existingLink) {
      ...
      if (!existingLink.ativo) {
        await supabaseClient.from("corretora_usuarios").update({ ativo: true }).eq("id", existingLink.id);
    ...
    if (existingUser) {
      ...
      const { data: existingProfile } = await supabaseClient.from("profiles").select("id").eq("id", userId).single();
      ...
      if (!existingProfile) {
        ...
        await supabaseClient.from("profiles").update({ status: "ativo" }).eq("id", userId);
    ...
    const { error: corretoraError } = await supabaseClient.from("corretora_usuarios").insert({
    ...
    try {
      ...
      if (template) {
        ...
        if (RESEND_API_KEY) {
          ...
          const fromEmail = resendConfig?.from_email || "vangard@uon1.com.br";
