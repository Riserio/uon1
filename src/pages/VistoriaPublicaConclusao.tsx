  ...
  const loadData = async () => {
    try {
      ...
      const { data: vistoriaData, error: vistoriaError } = await supabase
        .from("vistorias")
        // 🔑 Agora também traz o ID da corretora
        .select("*, corretoras(id, nome, logo_url)")
        .eq("link_token", token)
      ...
      // 🔒 Buscar termos EXCLUSIVAMENTE da corretora usada na abertura
      let query = supabase.from("termos").select("*").eq("ativo", true);

      if (vistoriaData.corretoras?.id) {
        query = query.eq("corretora_id", vistoriaData.corretoras.id);
      }

      const { data: termosData, error: termosError } = await query.order("ordem");
