-- Corrige importação completa de eventos (API Hinova):
-- limita a janela inicial a 18 meses para não estourar o timeout.
DROP FUNCTION IF EXISTS public.importar_eventos_api(uuid, boolean);
CREATE OR REPLACE FUNCTION public.importar_eventos_api(p_corretora_id uuid, p_full boolean DEFAULT false, p_desde date DEFAULT NULL)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_full OR v_imp IS NULL THEN
    -- Limite de janela no modo completo: sem histórico, buscar apenas os
    -- últimos 18 meses. Antes iniciava em 2010 => ~190 janelas HTTP e a
    -- função estourava o timeout, revertendo tudo (associações novas, como
    -- as AASC, nunca conseguiam popular eventos).
    v_desde := GREATEST(
      COALESCE(p_desde,
               (SELECT min(data_cadastro_evento) FROM sga_eventos e JOIN sga_importacoes si ON si.id=e.importacao_id WHERE si.corretora_id=p_corretora_id),
               v_fim - 540),
      COALESCE(p_desde, v_fim - 540));
    v_novo := true;
