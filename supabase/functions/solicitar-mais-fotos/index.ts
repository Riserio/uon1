  ...
  try {
    ...
    if (vistoria.cliente_email) {
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          from: "Vistoria Digital <vangard@uon1.com.br>",
    ...
    return new Response(JSON.stringify({ success: true, message: "Solicitação enviada", link: linkVistoria }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (error: any) {
    console.error("Error in solicitar-mais-fotos:", error);
    return new Response(JSON.stringify({ error: "Erro ao solicitar fotos", details: error.message }), {
      status: 500,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }
