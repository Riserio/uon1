...
export default function DashboardAnalytics() {
  ...
  const statusDistribution = statusConfigs.map((config) => {
    const count = atendimentos.filter(
      (a) => a.status.toLowerCase() === config.nome.toLowerCase()
    ).length;
    return {
      name: config.nome,
      value: count,
  ...
  const avgTimeByStatus = statusConfigs.map((config) => {
    const statusAtendimentos = atendimentos.filter(
      (a) => a.status.toLowerCase() === config.nome.toLowerCase()
    );
    ...
    const totalHours = statusAtendimentos.reduce((acc, a) => {
      const hours = differenceInHours(
        new Date(),
        new Date(a.status_changed_at || a.created_at)
      );
      return acc + hours;
    }, 0);
  ...
  const overdueRate = statusConfigs.map((config) => {
    ...
    return {
      ...
      taxa: statusAtendimentos.length > 0
        ? Math.round((vencidos / statusAtendimentos.length) * 100)
        : 0,
      color: config.cor,
    };
  }).filter(Boolean);
  ...
  const statusFinalizadosSet = new Set(
    statusConfigs.filter(s => s.is_final).map(s => s.nome.toLowerCase())
  );
  
  const responsavelRanking: ResponsavelStats[] = Object.values(
    atendimentos.reduce((acc, a) => {
      ...
      if (a.data_concluido) {
        acc[key].concluidos++;
        const tempo = differenceInHours(
          new Date(a.data_concluido),
          new Date(a.created_at)
        );
        acc[key].tempoMedio = 
          (acc[key].tempoMedio * (acc[key].concluidos - 1) + tempo) / acc[key].concluidos;
      }

      // Check if overdue
      const statusConfig = statusConfigs.find(
        (c) => c.nome.toLowerCase() === a.status.toLowerCase()
      );
      if (statusConfig && statusConfig.prazo_horas > 0) {
        const hours = differenceInHours(
          new Date(),
      ...
      return acc;
    }, {} as Record<string, ResponsavelStats>)
  ).map((stats) => ({
    ...stats,
    tempoMedio: Math.round(stats.tempoMedio),
    taxaConclusao: stats.total > 0 
      ? Math.round((stats.concluidos / stats.total) * 100)
      : 0,
  }));

  // Sort rankings
  const topByVolume = [...responsavelRanking]
    .sort((a, b) => b.total - a.total)
    .slice(0, 10);

  const topBySpeed = [...responsavelRanking]
    .filter((r) => r.concluidos > 0)
    .sort((a, b) => a.tempoMedio - b.tempoMedio)
    .slice(0, 10);

  const topByCompletionRate = [...responsavelRanking]
    .sort((a, b) => b.taxaConclusao - a.taxaConclusao)
    .slice(0, 10);

  // Worst performers
  const worstByVolume = [...responsavelRanking]
    .sort((a, b) => a.total - b.total)
    .slice(0, 10);

  const worstBySpeed = [...responsavelRanking]
    .filter((r) => r.concluidos > 0)
    .sort((a, b) => b.tempoMedio - a.tempoMedio)
    .slice(0, 10);

  const worstByCompletionRate = [...responsavelRanking]
  ...
  const tempoMedioGeral = atendimentos
    .filter((a) => a.data_concluido)
    .reduce((acc, a) => {
      return acc + differenceInHours(
        new Date(a.data_concluido!),
        new Date(a.created_at)
      );
    }, 0) / (concluidos || 1);
  ...
  const handleExportPDF = () => {
    ...
    autoTable(doc, {
      ...
      body: [
        ...
        ['Atendimentos Concluídos', concluidos.toString()],
        ['Taxa de Conclusão', `${taxaConclusao}%`],
        ['Atendimentos Vencidos', vencidosTotal.toString()],
        ['Tempo Médio de Conclusão', `${Math.round(tempoMedioGeral)}h`],
      ],
    });
    ...
    autoTable(doc, {
      ...
      body: statusDistribution.map((s) => [
        s.name,
        s.value.toString(),
        `${Math.round((s.value / totalAtendimentos) * 100)}%`,
      ]),
    });
    ...
    autoTable(doc, {
      ...
      body: topByVolume.map((r) => [
        r.responsavel,
        r.total.toString(),
        r.concluidos.toString(),
        `${r.taxaConclusao}%`,
      ]),
    });
                      ...
                      {statusDistribution
                        ...
                        .map((status, index) => {
                          ...
                          return (
                            <div
                              key={index}
                              className="flex items-center justify-between p-3 rounded-lg border bg-card hover:bg-accent/50 transition-colors duration-200 group"
                            >
                              <div className="flex items-center gap-3 flex-1">
                                <div
                                  className="h-4 w-4 rounded-full flex-shrink-0 ring-2 ring-offset-2 ring-offset-background group-hover:scale-110 transition-transform"
            ...
            <Card className="border-border/40 shadow-sm">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={320}>
                  <BarChart data={statusDistribution} margin={{ top: 20, right: 20, left: 0, bottom: 20 }}>
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <BarChart data={avgTimeByStatus}>
                    ...
                    <Bar dataKey="tempoMedio" name="Tempo Médio (h)" radius={[8, 8, 0, 0]}>
                      {avgTimeByStatus.map((entry, index) => (
                ...
                <CardContent>
                  <div className="space-y-3">
                    {topByVolume.map((resp, index) => (
                      <div key={resp.responsavel} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors">
            ...
            <div className="mt-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <Card className="border-destructive/20">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {worstByVolume.map((resp, index) => (
                        <div key={resp.responsavel} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors border border-destructive/10">
                ...
                <Card className="border-destructive/20">
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      {worstByCompletionRate.map((resp, index) => (
                        <div key={resp.responsavel} className="flex items-center gap-3 p-2 rounded-lg hover:bg-muted/50 transition-colors border border-destructive/10">
                          <div className="w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm bg-destructive/10 text-destructive">
                            {index + 1}
                          </div>
