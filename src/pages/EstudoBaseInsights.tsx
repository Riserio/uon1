...
export default function EstudoBaseInsights() {
  ...
  const fetchRegistros = useCallback(async () => {
    ...
    try {
      const {
        data: importacao,
        error: impError
      } = await supabase.from("estudo_base_importacoes").select("*").eq("ativo", true).eq("corretora_id", selectedAssociacao).order("created_at", {
        ascending: false
      }).limit(1).maybeSingle();
      ...
      if (importacao) {
        ...
        while (hasMore) {
          const {
            data: batch,
            error
          } = await supabase.from("estudo_base_registros").select("*").eq("importacao_id", importacao.id).range(offset, offset + BATCH_SIZE - 1);
          if (error) {
            console.error(error);
            break;
          }
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('estudo-base-realtime').on('postgres_changes', {
      event: '*',
      schema: 'public',
  ...
  return <div className="min-h-screen bg-background">
      <BIPageHeader title="Estudo de Base" subtitle="Análise detalhada da base de veículos e associados" associacoes={associacoes} selectedAssociacao={selectedAssociacao} onAssociacaoChange={setSelectedAssociacao} loadingAssociacoes={loadingAssociacoes} currentModule="estudo-base" showHistorico={canViewHistorico} onHistoricoClick={() => setHistoricoDialogOpen(true)} recordCount={registros.length} fileName={importacaoAtiva?.nome_arquivo} />
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          <div className="w-full overflow-x-auto pb-2">
            <TabsList className="inline-flex md:flex md:w-auto gap-1 p-1.5 bg-muted/40 rounded-xl min-w-max md:min-w-0">
              {tabs.map(tab => {
              const Icon = tab.icon;
              return <TabsTrigger key={tab.id} value={tab.id} className="flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-medium
          ...
          <TabsContent value="tabela" className="space-y-4 mt-0">
            <div className="overflow-x-auto">
              <table className="w-full text-xs border">
                <thead>
                  <tr className="border-b bg-muted/50">
                    {["Placa", "Tipo", "Montadora", "Modelo", "Categoria", "Ano", "Situação", "Valor Protegido", "Cooperativa", "Sexo", "Idade", "Data Contrato"].map(h => <th key={h} className="text-left py-2 px-2 font-medium whitespace-nowrap">{h}</th>)}
                  </tr>
                </thead>
                <tbody>
                  {registros.slice(0, 500).map((r, i) => <tr key={r.id || i} className="border-b hover:bg-muted/30">
                      ...
                      <td className="py-1.5 px-2 whitespace-nowrap">{r.valor_protegido ? `R$ ${Number(r.valor_protegido).toLocaleString('pt-BR', {
              ...
              {registros.length > 500 && <p className="text-xs text-muted-foreground mt-2 text-center">Exibindo 500 de {registros.length.toLocaleString('pt-BR')} registros</p>}
            </div>
          </TabsContent>

          <TabsContent value="importar" className="space-y-4 mt-0">
            <EstudoBaseImportacao onImportSuccess={() => {
            fetchRegistros();
            setActiveTab("dashboard");
          }} corretoraId={selectedAssociacao} corretoraNome={selectedAssociacaoNome} />
          </TabsContent>
        </Tabs>
      </div>
