...
serve(async (req) => {
    if (req.method === 'OPTIONS') {
      ...
      const [evRes, cobRes] = await Promise.all([
              invocar('gerar-resumo-eventos'),
              invocar('gerar-resumo-cobranca'),
            ]);
      ...
      if (ev) {
              ...
              📍 Cidade com mais eventos: *${ev.cidade_mais_eventos}*
              🏢 Cooperativa com mais eventos: *${ev.cooperativa_mais_eventos}*`);
      }
      ...
      if (cob) {
              ...
              🔴 Maior inadimplência: *${cob.cooperativa_maior_inadimplencia}*
              🟢 Menor inadimplência: *${cob.cooperativa_menor_inadimplencia}*`);
      }
      ...
      const dados: Record<string, any> = {
              ...
              ev_cidade_top: ev?.cidade_mais_eventos ?? '-',
              ev_cooperativa_top: ev?.cooperativa_mais_eventos ?? '-',
              // Cobrança
              cob_mes_referencia: cob?.mes_referencia ?? '-',
              cob_data_atual: cob?.data_atual ?? '-',
              cob_percentual_inadimplencia: cob?.percentual_inadimplencia ?? '-',
              cob_total_gerados: cob?.total_gerados ?? '-',
              cob_total_baixados: cob?.total_baixados ?? '-',
              cob_total_inadimplentes: cob?.total_inadimplentes ?? '-',
              cob_faturamento_esperado: cob?.faturamento_esperado_formatado ?? '-',
              cob_faturamento_recebido: cob?.faturamento_recebido_formatado ?? '-',
              cob_total_aberto: cob?.total_aberto_formatado ?? '-',
              cob_boletos_por_dia: cob?.boletos_por_dia ?? '-',
              cob_coop_maior_inadimplencia: cob?.cooperativa_maior_inadimplencia ?? '-',
              cob_coop_menor_inadimplencia: cob?.cooperativa_menor_inadimplencia ?? '-',
              // MGF
