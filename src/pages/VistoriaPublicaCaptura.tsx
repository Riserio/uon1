...
export default function VistoriaPublicaFormulario() {
  ...
  // 🔧 AQUI É ONDE A GENTE LIGA O OCR NOS CAMPOS DO FORMULÁRIO
  const loadTempData = () => {
    ...
    // Dados da CNH (nome, CPF, condutor)
    if (data?.cnhData) {
      ...
      setFormData((prev) => ({
        ...
        // 👉 condutor preenche com o mesmo nome do documento
        condutor_veiculo: nomeFromCnh || prev.condutor_veiculo,
      }));
    }

    // Dados do veículo (placa, modelo – podendo incluir marca + modelo)
    if (data?.vehicleData) {
      const v = data.vehicleData;

      const placaFromOcr = v.placa || v.placa_veiculo || v.vehicle_plate;

      const modeloFromOcr =
        v.modelo || v.modelo_veiculo || v.marca_modelo || (v.marca && v.modelo ? `${v.marca} ${v.modelo}` : undefined);
