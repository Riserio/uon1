-- 1) Worker de enriquecimento passa a usar tambem a BASE ATIVA (estudo_base_registros)
--    como fonte de cooperativa/regional, alem de cadastro e eventos.
...
BEGIN
  ...
  -- NOVO: base ativa (estudo de base) — cobre associacoes cujo cadastro/eventos
  -- nao trazem cooperativa para todas as placas (caso AASC).
  CREATE TEMP TABLE IF NOT EXISTS _lk_eb (corretora_id uuid, placa_n text, cooperativa text, regional text) ON COMMIT DROP;
  DELETE FROM _lk_eb;
  INSERT INTO _lk_eb
  SELECT DISTINCT ON (bi.corretora_id, x.placa_n) bi.corretora_id, x.placa_n, x.cooperativa, x.regional
  FROM (
    SELECT DISTINCT ON (corretora_id) corretora_id, id FROM estudo_base_importacoes WHERE ativo ORDER BY corretora_id, created_at DESC
  ) bi
  JOIN LATERAL (
    SELECT upper(regexp_replace(COALESCE(br.placa,''), '[^A-Za-z0-9]', '', 'g')) AS placa_n,
           NULLIF(br.cooperativa,'') AS cooperativa, NULLIF(br.regional,'') AS regional
    FROM estudo_base_registros br
    WHERE br.importacao_id = bi.id
      AND (NULLIF(br.cooperativa,'') IS NOT NULL OR NULLIF(br.regional,'') IS NOT NULL)
  ) x ON x.placa_n <> '';
  CREATE INDEX IF NOT EXISTS _lk_eb_idx ON _lk_eb (corretora_id, placa_n);
  ...
  SET cooperativa = COALESCE(NULLIF(cb.cooperativa,''), lp.cooperativa, lc.cooperativa, le.cooperativa, lb.cooperativa),
      regional_boleto = COALESCE(NULLIF(cb.regional_boleto,''), lp.regional, lc.regional, le.regional, lb.regional),
  ...
  LEFT JOIN _lk_eb lb ON lb.corretora_id = a.corretora_id AND a.placa_n <> '' AND lb.placa_n = a.placa_n
  WHERE cb.id = a.id;
