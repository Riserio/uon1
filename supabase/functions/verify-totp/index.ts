...
export default function Auth() {
  ...
  const checkTOTPStatus = async (currentUser?: MinimalUser) => {
    ...
    try {
      // 1. Busca o status TOTP no Banco de Dados
      ...
      if (error) {
        ...
        toast.error("Erro ao verificar autenticação no DB");
        return;
      }

      // 2. Se não estiver configurado, inicia o SETUP
  ...
  const setupTOTP = async (currentUser?: MinimalUser) => {
    ...
    if (!effectiveUser?.email) {
      console.error("Email do usuário não disponível para setup TOTP.");
      return;
    }

    try {
      // A Edge Function 'verify-totp/setup' retorna o QR Code
      ...
      // CORREÇÃO: Trata o erro da Edge Function.
      // Se a Edge Function retornar Status 4xx ou 5xx, o Supabase SDK encapsula em um objeto 'error'.
      if (error) {
        // A Edge Function deve retornar um JSON com 'error' para facilitar a depuração.
      ...
      // Este bloco captura tanto erros do invoke quanto erros da Edge Function (Status != 2xx)
      console.error("Error setting up TOTP:", error.message || error);
      toast.error(`Erro ao configurar autenticação: ${error.message || "Tente novamente."}`);
    }
  };
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.isParceiro) {
        ...
        setLoginPhase("totp");
        // Inicia o fluxo de verificação/setup TOTP
  ...
  const handleVerifyTotp = async (e: React.FormEvent) => {
    ...
    try {
      // Rota de Verificação (Chama /verify-totp)
      ...
      if (error || !data?.valid) {
        // Se a Edge Function retornar 400 ou 500, 'error' será preenchido.
        // Se retornar 200, mas com { valid: false }, o código é inválido.
  ...
  const handleSetupTotp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        // Trata erro de Edge Function (4xx ou 5xx)
        const errorMessage = error.message || "Erro ao verificar código.";
        throw new Error(errorMessage);
      }

      if (data.valid) {
        // Se o código for válido, ativa o TOTP no DB
      ...
      console.error("TOTP setup error:", error.message || error);
            ...
            <form onSubmit={handleSetupTotp} className="space-y-4">
              <div className="flex flex-col items-center space-y-4">
                <div className="p-4 bg-white rounded-lg border-2 border-border">
                  {/* Se qrCodeUri estiver disponível, mostra o QR Code */}
                  {qrCodeUri ? (
                    <img
