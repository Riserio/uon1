...
const getFeriadosBH = (year: number): string[] => {
  ...
  `${year}-04-21`,
  // Data Magna de Minas Gerais (mesmo dia de Tiradentes)
  ...
  feriados.push(formatDate(carnaval),
  // Segunda de Carnaval
  formatDate(carnaval2),
  // Terça de Carnaval
...
export default function GestaoJornada() {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("registros_ponto").select("*").eq("funcionario_id", funcionarioId).gte("data_hora", inicio).lte("data_hora", fim).order("data_hora", {
        ascending: false
      });
      if (error) throw error;
      return data;
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const {
        data,
        error
      } = await supabase.from("registros_ponto").select("*, funcionarios(nome, cargo)").gte("data_hora", inicio).lte("data_hora", fim).order("data_hora", {
        ascending: false
      });
      if (error) throw error;
      return data;
  ...
  const detailedStats = useMemo(() => {
    ...
    Object.entries(registrosPorDia).forEach(([dia, regs]) => {
      ...
      workedDays.push({
        date: dia,
        dayName: format(parseISO(dia), "EEEE", {
          locale: ptBR
        }),
        hoursWorked,
        isLate,
  ...
  const exportToPDF = (individual: boolean) => {
    ...
    doc.text(`Período: ${format(new Date(ano, mes - 1), "MMMM 'de' yyyy", {
      locale: ptBR
    })}`, 20, 28);
    ...
    if (!dataToExport || dataToExport.length === 0) {
      ...
      const tableData = dataToExport.map((r: any) => [individual ? "" : r.funcionarios?.nome || "N/A", format(new Date(r.data_hora), "dd/MM/yyyy"), format(new Date(r.data_hora), "EEEE", {
        locale: ptBR
      }), format(new Date(r.data_hora), "HH:mm"), tiposPonto.find(t => t.value === r.tipo)?.label || r.tipo]);
  ...
  const exportToExcel = (individual: boolean) => {
    ...
    const excelData = dataToExport.map((r: any) => ({
      ...
      Dia: format(new Date(r.data_hora), "EEEE", {
        locale: ptBR
      }),
  ...
  const baixarReciboPDF = () => {
    ...
    doc.text("RECIBO DE PONTO", pageWidth / 2, yPosition, {
      align: "center"
    });
    yPosition += 10;
    ...
    doc.text(`Protocolo: ${protocolo}`, pageWidth / 2, yPosition, {
      align: "center"
    });
    yPosition += 15;
    ...
    doc.setFont("helvetica", "normal");
    doc.text(format(new Date(), "EEEE, dd 'de' MMMM 'de' yyyy", {
      locale: ptBR
    }), margin + 15, yPosition);
    yPosition += 15;
    ...
    yPosition += 10;
    const sortedRecords = todayRecords.sort((a: any, b: any) => new Date(a.data_hora).getTime() - new Date(b.data_hora).getTime());
    ...
    doc.text(`Documento gerado em ${format(new Date(), "dd/MM/yyyy 'às' HH:mm:ss", {
      locale: ptBR
    })}`, margin, yPosition);
    yPosition += 4;
  ...
  const enviarReciboWhatsApp = () => {
    ...
    const protocolo = gerarProtocolo();
    const registrosTexto = todayRecords.sort((a: any, b: any) => new Date(a.data_hora).getTime() - new Date(b.data_hora).getTime()).map((r: any) => {
  ...
  return <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {isLimitedUser ? "Registre e acompanhe sua jornada de trabalho" : "Registre e acompanhe a jornada de trabalho dos colaboradores"}
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {funcionarioId && <Tabs defaultValue="registro" className="space-y-4">
          <TabsList>
          ...
          <TabsContent value="registro">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {location ? <span className="flex items-center gap-1 text-green-600">
              ...
              <CardContent>
                ...
                return <Button key={tipo.value} variant="outline" className={`h-24 flex-col gap-2 ${tipo.color} ${isRegistered ? 'opacity-50 cursor-not-allowed' : ''}`} onClick={() => registrarPonto.mutate(tipo.value)} disabled={registrarPonto.isPending || isRegistered} title={isRegistered ? `${tipo.label} já registrado(a) hoje` : `Registrar ${tipo.label}`}>
                        <Icon className="h-8 w-8" />
                        <span>{tipo.label}</span>
                        {isRegistered && <span className="text-xs text-muted-foreground">Registrado</span>}
                      </Button>;
              })}
                </div>
                ...
                <div className="mt-6 p-4 bg-muted/50 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {todayRecords.length > 0 && <DropdownMenu>
                  ...
                  return <div key={registro.id} className="flex items-center justify-between p-2 bg-background rounded">
                    ...
                    {todayRecords.length === 0 && <p className="text-muted-foreground text-sm text-center py-2">
          ...
          <TabsContent value="historico">
            <Card>
                ...
                <div className="flex items-center justify-between flex-wrap gap-4">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <Select value={ano.toString()} onValueChange={v => setAno(parseInt(v))}>
                      ...
                      <SelectContent>
                        {[2024, 2025, 2026].map(a => <SelectItem key={a} value={a.toString()}>
                            {a}
                          </SelectItem>)}
                      </SelectContent>
                    </Select>
              ...
              <CardContent>
                {isLoading ? <div className="text-center py-8 text-muted-foreground">Carregando...</div> : Object.keys(registrosPorDia || {}).length === 0 ? <div className="text-center py-8 text-muted-foreground">
                    Nenhum registro encontrado neste período
                  </div> : <div className="space-y-4">
                    {Object.entries(registrosPorDia || {}).sort((a, b) => b[0].localeCompare(a[0])).map(([dia, regs]: [string, any]) => <div key={dia} className="border rounded-lg p-4">
                        <div className="flex items-center justify-between mb-2">
                          <span className="font-medium">
                            {format(parseISO(dia), "EEEE, dd 'de' MMMM", {
                      locale: ptBR
                    })}
                          </span>
