...
export default function AcompanhamentoSinistroInterno() {
  ...
  const handleChangeCorretora = async (corretoraId: string | null) => {
    ...
    try {
      const { error } = await supabase.from("atendimentos").update({ corretora_id: corretoraId }).eq("id", id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (data.id) {
        const { error } = await supabase.from("sinistro_acompanhamento").update(payload).eq("id", data.id);
  ...
  const handleSendToCilia = async () => {
    ...
    try {
      ...
      console.error("Erro ao enviar para CILIA (try/catch):", error);
  ...
  const handleSendToSga = async () => {
    ...
    try {
      ...
      console.log("enviar-sga-hinova result:", { result, error });

      if (error) {
        console.error("Erro retornado pela Edge Function enviar-sga-hinova:", error);
        const msg = (error as any)?.message?.includes("non-2xx status")
          ? "A função enviar-sga-hinova retornou um status diferente de 2xx. Verifique os logs da Edge Function."
          : (error as any)?.message || "Erro ao sincronizar com SGA Hinova";
      ...
      console.error("Erro ao enviar para SGA Hinova (try/catch):", error);
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold">
                  Acompanhamento - SIN-
                  {new Date(atendimento.created_at).getFullYear()}-{String(atendimento.numero).padStart(6, "0")}
                </h1>
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setData({
                          ...data,
                          reparo_data_autorizacao: e.target.value,
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <CurrencyInput
                      ...
                      onValueChange={(values) =>
                        setData({
                          ...data,
                          financeiro_valor_aprovado: values.floatValue || 0,
                        })
                      }
                      placeholder="R$ 0,00"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <CurrencyInput
                      ...
                      onValueChange={(values) =>
                        setData({
                          ...data,
                          financeiro_valor_pago: values.floatValue || 0,
                        })
                      }
                      placeholder="R$ 0,00"
                    />
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setData({
                          ...data,
                          financeiro_data_pagamento: e.target.value,
                        })
                      }
                    />
                  </div>
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setData({
                              ...data,
                              desistencia_data: e.target.value,
                            })
                          }
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            setData({
                              ...data,
                              desistencia_motivo: e.target.value,
                            })
                          }
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setData({
                              ...data,
                              finalizado_data: e.target.value,
                            })
                          }
                        />
                      </div>
                      <div className="space-y-2">
                        ...
                        <Textarea
                          ...
                          onChange={(e) =>
                            setData({
                              ...data,
                              finalizado_observacoes: e.target.value,
                            })
                          }
                    ...
                    <div>
                      ...
                      {data.cilia_enviado ? (
                        <Badge variant="outline" className="bg-green-500/10 text-green-600 border-green-500/30">
                          <CheckCircle2 className="h-3 w-3 mr-1" />
                          Enviado em{" "}
                          {data.cilia_enviado_em
                            ? format(new Date(data.cilia_enviado_em), "dd/MM/yyyy 'às' HH:mm", { locale: ptBR })
                            : ""}
                        </Badge>
                      ) : (
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <p className="text-sm text-muted-foreground">
                    A sincronização com o SGA Hinova só está disponível após a finalização do sinistro. Todos os dados
                    do acompanhamento serão enviados ao sistema.
                  </p>
                  <div className="flex items-center justify-between">
                    <div>
