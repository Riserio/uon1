          ...
          <div className="w-full overflow-x-auto pb-2 -mx-1 px-1 flex justify-center">
            <TabsList className="inline-flex md:flex md:w-full gap-1 p-1.5 bg-muted/50 rounded-full min-w-max md:min-w-0 shadow-sm mx-auto">
              {tabs.map((tab) => {
                const Icon = tab.icon;
                return (
                  <TabsTrigger key={tab.id} value={tab.id} className="flex items-center gap-2 px-3.5 sm:px-4 py-2.5 rounded-full text-sm font-medium text-muted-foreground transition-all data-[state=active]:bg-primary data-[state=active]:text-primary-foreground data-[state=active]:shadow-md hover:text-foreground hover:bg-background/50 whitespace-nowrap">
