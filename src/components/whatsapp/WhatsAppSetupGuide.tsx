...
export function WhatsAppSetupGuide({ onNavigate }: { onNavigate: (aba: string) => void }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const cfg = await supabase.from("whatsapp_config").select("id", { count: "exact", head: true }).eq("ativo", true);
        const tpl = await supabase.from("whatsapp_templates").select("id", { count: "exact", head: true });
        const flows = await supabase.from("whatsapp_flows").select("id", { count: "exact", head: true }).eq("ativo", true);
